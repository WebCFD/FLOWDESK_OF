...
echo ""
runApplication cartesianMesh -parallel -np 3
echo "==================== DETECTING MESH LOCATION ===================="
python3 << 'MESH_DETECT_EOF'
import os
# Detect if mesh is in serial (constant/polyMesh) or parallel (processor0/constant/polyMesh)
if os.path.exists('constant/polyMesh/boundary'):
    mesh_location = 'constant'
    print('✓ Mesh found in SERIAL location: constant/polyMesh')
elif os.path.exists('processor0/constant/polyMesh/boundary'):
    mesh_location = 'processor0/constant'
    print('✓ Mesh found in PARALLEL location: processor0/constant/polyMesh')
else:
    print('✗ ERROR: Mesh not found in constant/polyMesh or processor0/constant/polyMesh')
    exit(1)
with open('/tmp/mesh_location.txt', 'w') as f:
    f.write(mesh_location)
MESH_DETECT_EOF
MESH_LOCATION=$(cat /tmp/mesh_location.txt)
...
expected_patches = ['wall_0F_1', 'door_0F_1', 'wall_0F_4', 'wall_0F_5', 'wall_0F_6', 'window_0F_1', 'floor_0F', 'ceil_0F']
...
if failed:
    ...
    print()
    print('Expected patches: wall_0F_1, door_0F_1, wall_0F_4, wall_0F_5, wall_0F_6, window_0F_1, floor_0F, ceil_0F')
